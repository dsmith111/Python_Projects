...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self,cat,obs,used):
        prompt = '{} has ${}\nCategory: {}\nPhrase:  {}\nGuessed: {}'
        print(prompt.format(self.name,self.prizeMoney,cat,obs,used))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,used):
        vowel = self.prizeMoney>=VOWEL_COST
        x = [l for l in LETTERS if l not in used]
        if vowel == False:
            x = [l for l in x if l not in VOWELS]
        x= "".join(x)
        return x
                
    def getMove(self,cat,obs,used):
        usable = self.getPossibleLetters(used)
        if len(usable) ==0:
                return 'pass'
        smart = self.smartCoinFlip()
        if smart:
            move = [l for l in self.SORTED_FREQUENCIES if l in usable]
            move = move[0]
        else:
